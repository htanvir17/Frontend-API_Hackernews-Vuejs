...
<div>
    <div  v-for="comment in node" :key=comment>
                    ...
                    <span class="commtext c00">
                      ...
                      <div class="reply">
                        <p>
                          <font size="1">
                            <u>
                              <a v-if="comment.author != user.username" :href="'/reply?id='+ comment.contribution_ref +'&idc='+ comment.id_contribution">reply</a>
                            </u>
                          </font>
                        </p>
                      </div>
      ...
      <!--recursion-->
      <tree v-if="comment.replies && comment.replies.length" :node="comment.replies"></tree>
...
<script>
  ...
  export default {
      name: "tree",
      //with this properties, we are taking the data that is sending from Threads.vue 
      props: {
        node: Object //name has to be 'node' because this is how we specified in Threads.vue
      },
      data(){
            return{
                user:{
